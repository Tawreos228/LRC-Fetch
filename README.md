...
> **Главное отличие — находимость.** Если точного совпадения по тегам нет,
> LRC Fetch не останавливается на «текста нет», а идёт в полнотекстовый поиск
> и ко второму источнику. Тексты, которые загрузчики «только по точному
> совпадению» числят отсутствующими, здесь находятся.

## Возможности

- **Находит больше** — при отсутствии точного совпадения идёт в поиск и
  подбирает лучший синхронный вариант по длительности, а не сдаётся на первом
  же промахе. Именно здесь появляются тексты, которых «нет» у других.
